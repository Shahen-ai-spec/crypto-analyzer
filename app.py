        ...
        if not api_key:
            ...
            with st.spinner("Γίνεται ανάλυση με το Gemini 3.6 Flash..."):
                try:
                    ...
                    prompt = """
Είσαι ένας αυστηρός Technical Analyst.
Διάβασε τα δεδομένα από την εικόνα ακολουθώντας ΑΥΣΤΗΡΑ τους παρακάτω κανόνες:

1. **Pair**: Διάβασε το ζεύγος ΑΚΡΙΒΩΣ όπως αναγράφεται πάνω αριστερά στο chart (π.χ. SUI/USDC).
2. **Direction**: 
   - Αν ο RSI είναι πάνω από 70, η τάση θεωρείται Overbought -> SHORT.
   - Αν ο RSI είναι κάτω από 30, θεωρείται Oversold -> LONG.
3. **Entry Price**: 
   - Χρησιμοποίησε ΑΚΡΙΒΩΣ την τιμή κλεισίματος (C / Close) που αναγράφεται πάνω αριστερά δίπλα στο pair (π.χ. C0.7306).
4. **Stop Loss (SL) & Take Profit (TP)**:
   - Αν υπάρχει εργαλείο Long/Short Position στο chart, διάβασε ΑΚΡΙΒΩΣ τους αριθμούς του.
   - Αν ΔΕΝ υπάρχει εργαλείο, υπολόγισε μαθηματικά με βάση την τιμή Entry:
     * Για SHORT: SL = Entry * 1.02 (2% πάνω), TP1 = Entry * 0.96 (4% κάτω).
     * Για LONG:  SL = Entry * 0.98 (2% κάτω), TP1 = Entry * 1.04 (4% πάνω).
5. **RSI**: Διάβασε την τιμή που αναγράφεται ακριβώς δίπλα στο "RSI 14 close" (π.χ. 75.50).

ΜΗΝ μαντεύεις τιμές από τον δεξιό άξονα αν είναι θολές. Βασίσου αποκλειστικά στους αριθμούς του κειμένου (OHLC & RSI).
"""
                    response = None
