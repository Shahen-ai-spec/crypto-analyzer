...
4. **Stop Loss (SL) & Take Profit (TP)** (Πολύ στενά για scalping):
   ...
   - Αν ΔΕΝ υπάρχει εργαλείο, υπολόγισε μαθηματικά:
     * Για SHORT: SL = Entry * 1.008 (0.8% πάνω), TP1 = Entry * 0.988 (1.2% κάτω).
     * Για LONG:  SL = Entry * 0.992 (0.8% κάτω), TP1 = Entry * 1.012 (1.2% πάνω).
...
if e_val > 0 and s_val > 0 and e_val != s_val:
    ...
    c2.metric("Πιθανό Κέρδος", f"${potential_profil_usd:.2f}")
